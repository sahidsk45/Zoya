...
export default function Visualizer({ state, onToggle }: VisualizerProps) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none">
      ...
      {/* Core Circle - Now clickable and highly responsive! */}
      <motion.div
        animate={getPulseAnimation()}
        onClick={onToggle}
        className={`absolute w-[45%] h-[45%] sm:w-[35%] sm:h-[35%] md:w-[25%] md:h-[25%] rounded-full border-[2px] ${theme.border} bg-black/70 backdrop-blur-md flex flex-col items-center justify-center shadow-[inset_0_0_30px_rgba(0,0,0,0.5)] cursor-pointer pointer-events-auto select-none transition-all duration-300 hover:scale-105 active:scale-95`}
        style={{ boxShadow: `0 0 50px ${theme.color}, inset 0 0 30px ${theme.color}` }}
      >
        {/* Center Text */}
        <div 
          className="font-bold tracking-[0.3em] text-2xl md:text-3xl lg:text-4xl text-white"
          style={{ textShadow: `0 0 15px ${theme.color}, 0 0 30px ${theme.color}` }}
        >
          ZOYA
        </div>
        
        {/* Helper instruction underneath Zoya's name inside the core */}
        <div className="mt-2 text-[10px] md:text-xs font-semibold tracking-widest text-white/80 animate-pulse text-center px-2 uppercase">
          {state === "idle" && (
            <span className="text-[#00E5FF]">TAP TO START</span>
          )}
          {state === "listening" && (
            <span className="text-violet-400">LISTENING</span>
          )}
          {state === "processing" && (
